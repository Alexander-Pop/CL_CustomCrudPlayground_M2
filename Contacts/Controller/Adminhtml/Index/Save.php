...
use Codelegacy\Contacts\Model\Contacts;
  ...
  */
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            if (empty($data['codelegacy_contacts_id'])) {
                $data['codelegacy_contacts_id'] = null;
            }


            /** @var \Magento\Cms\Model\Block $model */
            $model = $this->_objectManager->create('Codelegacy\Contacts\Model\Contacts')->load($id);
            if (!$model->getId() && $id) {
                $this->messageManager->addError(__('This codelegacy_contacts no longer exists.'));
                return $resultRedirect->setPath('*/*/');
            }


            // if (isset($data['thumbnail_image'][0]['name']) && isset($data['thumbnail_image'][0]['tmp_name'])) {
            //     $data['thumbnail_image'] ='/codelegacy_contacts/'.$data['thumbnail_image'][0]['name'];
            ...
            //     $data['glow_image'] ='/codelegacy_contacts/'.$data['glow_image'][0]['name'];
            ...
            try {
                ...
                if ($this->getRequest()->getParam('back')) {
                    return $resultRedirect->setPath('*/*/edit', ['codelegacy_contacts_id' => $model->getId()]);
                }
                return $resultRedirect->setPath('*/*/');
            } catch (LocalizedException $e) {
                $this->messageManager->addError($e->getMessage());
            } catch (\Exception $e) {
                $this->messageManager->addException($e, __('Something went wrong while saving the codelegacy_contacts.'));
                print_r($e);
            }

            $this->dataPersistor->set('codelegacy_contacts', $data);
            return $resultRedirect->setPath('*/*/edit', ['codelegacy_contacts_id' => $this->getRequest()->getParam('codelegacy_contacts_id')]);
        }
        return $resultRedirect->setPath('*/*/');
    }
}
